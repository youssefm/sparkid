...
// Used for binary packing in toBytes.
...
export function toBytes(id: string): Uint8Array {
  ...
  for (let characterIndex = 0, byteIndex = 0; byteIndex < PACKED_GROUPS_BYTE_COUNT; characterIndex += CHARACTERS_PER_GROUP, byteIndex += BYTES_PER_GROUP) {
    const packed =
  ...
  out[PACKED_GROUPS_BYTE_COUNT] = d[id.charCodeAt(TAIL_CHARACTER_INDEX)] << TAIL_SHIFT;
  return out;
}
...
export function fromBytes(bytes: Uint8Array): string {
  ...
  for (let characterIndex = 0, byteIndex = 0; byteIndex < PACKED_GROUPS_BYTE_COUNT; characterIndex += CHARACTERS_PER_GROUP, byteIndex += BYTES_PER_GROUP) {
    const packed = (bytes[byteIndex] << 16) | (bytes[byteIndex + 1] << 8) | bytes[byteIndex + 2];
  ...
  const lastIndex = (bytes[PACKED_GROUPS_BYTE_COUNT] >>> TAIL_SHIFT) & INDEX_MASK;
  if (lastIndex >= BASE || (bytes[PACKED_GROUPS_BYTE_COUNT] & PADDING_MASK) !== 0) {
    throw new RangeError(
